...
<script>
import DialogFound from './DialogFound'
export default {
  data() {
    return {
      tableData: [],
  ...
  methods: {
    ...
    // 分页
      ...
      let sortnum = this.paginations.page_size * (page - 1);  //5*(3-1)前2页的条数
      let table = this.allTableData.filter((item, index) => {    //所有条数过滤一下
        console.log(index);
        return index >= sortnum;   // 过滤返回除最后一页的条数
      });
      // 设置默认分页数据
      // console.log(table);
    ...
    handleSizeChange(page_size) {
      ...
      this.tableData = this.allTableData.filter((item,index) => {
        return index < page_size;      //应该是过滤返回一页的条数
      });
    },
setPaginations() {
      // 总页数
